...
function FileRow({ fileName, fileExtention, path, setSelected, selected }) {
  const [url, setUrl] = useState(null);
  const [imageSrc, setImageSrc] = useState("");
  const [extention, setExtenstion] = useState(null);
  ...
  const selectingFileIcon = useCallback(
    (fileExt) => {
      if (imageExtensions.some((ext) => ext === fileExt)) {
        setExtenstion("Image");
        setImageSrc("/filesIcons/img.svg");
      } else if (fileExt === "pdf") {
        setExtenstion("PDF");
        setImageSrc("/filesIcons/pdf.svg");
      } else if (fileExt === "txt") {
        setExtenstion("Text file");
        setImageSrc("/filesIcons/txt.svg");
      } else {
        setExtenstion("File");
        setImageSrc("/filesIcons/file.svg");
      }
    },
    [imageExtensions, setExtenstion, setImageSrc]
  );

  useEffect(() => {
    selectingFileIcon(fileExtention);
    ...
    fetchDownloadUrl();
  }, [fileName, path, fileExtention, selectingFileIcon]);
  ...
  return (
    <div
      className={`group  flex  cursor-default justify-center flex-col w-full pl-[1.3rem] pr-4 ${
        selected?.name === fileName ? "bg-[#C2E7FF]" : "hover:bg-neutral-200"
      } `}
      ...
      <div className="flex items-center justify-between py-1 ">
        <div className="flex w-1/4 min-w-52 mr-4">
          <div className="flex gap-4 justify-center items-center ">
            {imageSrc && (
              <Image src={imageSrc} width={15} height={15} alt="Icon" />
            )}
            <h5 className="truncate">{fileName}</h5>
          </div>
        </div>
        <div className="flex justify-between items-center">
          <div className="flex justify-start items-center gap-20 2xl:gap-32">
            ...
            <div className=" truncate  px-3 py-1 ">
